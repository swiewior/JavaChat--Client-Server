...
public class ListViewCanvas extends Canvas implements CommonSettings
{
	...
	protected void SendDirectMessage()
	{
		...
		if (SelectedUser.equals(""))
		{
			chatclient.messagecanvas.AddMessageToMessageObject(
				"Nieprawidłowy wybór użytkownika",MESSAGE_TYPE_ADMIN);
			return;
		}
		if (SelectedUser.equals(chatclient.UserName))
		{
		...
		protected void SendFile()
	{
		...
		{
			chatclient.messagecanvas.AddMessageToMessageObject(
				"Nie możesz wysłać pliku do siebie",MESSAGE_TYPE_ADMIN);	
			return;
		}	
		// Wybór pliku
	...
	{	
		...
		{
			messageobject = (MessageObject) ListArray.get(G_ILoop);
			if((CurrentY >= messageobject.StartY) && 
				(CurrentY <= (messageobject.StartY+DEFAULT_LIST_CANVAS_HEIGHT)))
			{		
	...
	private void CreatePrivateWindow()
	{
		...
		if(!(IsIgnoredUser(SelectedUser)))	
		{
			...
			{	
				...
				{
					chatclient.messagecanvas.AddMessageToMessageObject(
						"Przekroczenie limitu prywatnych wiadomości",MESSAGE_TYPE_ADMIN);	
				}
				else				
				{
					chatclient.privatewindow[chatclient.PrivateWindowCount++] = 
						new PrivateChat(chatclient,SelectedUser);		
	...
	{		
		...
		if ((offGraphics == null) || (dimension.width != offDimension.width)|| 
